...
def runScript():
   ...
   log = open('log_classification_'+time.strftime("%Y-%m-%d_%H%M", time.localtime())+'.txt','w')
   class Unbuffered:
   ...
   print('----------')
   print(time.strftime("%H:%M:%S", time.localtime()),'--- STARTING CLASSIFICAION SCRIPT ---')
   ...
   #if classify image is chosen, open image and print information
   ...
   if clfMode == 1:
       ...
       print(the_time(),'---- Setting training variables and labels')
       ...
       if maxfeatures_user:
           if maxfeatures_user in ['auto','sqrt','log2']:
               clf1.set_params(max_features=maxfeatures_user)
           if '.' in maxfeatures_user:
               clf1.set_params(max_features=float(maxfeatures_user))
           else:
               clf1.set_params(max_features=int(maxfeatures_user)) 
       if njobs_user:
   ...
   #if classify image is chosen, open image in blocks and predict class and probabilities
   if inputType == 1 or inputType == 3:

       print(the_time(), '--- Reading and Classifying Image ---')
       ...
       print(the_time(),'---- Raster created')
       print(the_time(),'---- Image outputs:')
       print(the_time(),'------ Class output:',os.path.basename(outRaster))
       print(the_time(),'------ Probabilities output:',os.path.basename(outRaster2))
       print(the_time(),'---')

   #if classify CSV/validation is chosen, predict class based on csv
   if inputType == 2 or inputType == 3:

       print(the_time(),'---- Reading Validation/CSV')
       print(time.strftime("%H:%M:%S", time.localtime()),'---- Validation/CSV data read')
        
       df_val = pd.read_csv(input_csv,sep=',')
       val_data = df_val[csv_vars]
       print(the_time(),'---- Predicting Validation/CSV')
       val_pred = clf1.predict(val_data)
       if not no_label:
           val_label = df_val[csv_labels]
           val_score = accuracy_score(val_label,val_pred)
           print(the_time(),'---- Validation Overall Accuracy:', round(val_score,4))
           out_val_df = pd.DataFrame()
           out_val_df['Predicted'] = val_pred
           out_val_df['Actual'] = val_label
           out_val_df.to_csv(out_csv_pred,sep=';',index=False)
           print(the_time(),'------ Csv prediction file created:',os.path.basename(out_csv_pred))
           del out_val_df
           confusion_matrix = pd.crosstab(val_label, val_pred, rownames=['Actual'], colnames=['Predicted'])
           confusion_matrix.to_csv(out_csv_cmatrix,sep=';') 
           print(the_time(),'------ Confusion matrix file created:',os.path.basename(out_csv_cmatrix))
           print(the_time(),'---- *Notice: Rows = Actual Labels, Columns = Predicted Labels')
       else:
           pd.DataFrame({"Predicted":val_pred}).to_csv(out_csv_pred,sep=';',index=False)
           print(the_time(),'------ Csv prediction file created:',os.path.basename(out_csv_pred))
